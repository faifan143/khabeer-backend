...
export class BusinessFlowNotificationsService {
     ...
     */
    async notifyPaymentSuccess(orderId: number, customerId: number, providerId: number, amount: number) {
        try {
     ...
     */
    async notifyPaymentFailure(orderId: number, customerId: number, errorMessage: string) {
        try {
            const customer = await this.prisma.user.findUnique({
                where: { id: customerId },
                select: { name: true, fcm: true }
     ...
     */
    async notifyNewRating(ratingId: number, providerId: number, customerName: string, rating: number, comment?: string) {
        try {
            const provider = await this.prisma.provider.findUnique({
                where: { id: providerId },
                select: { name: true, fcm: true }
     ...
     */
    async notifyRatingUpdate(ratingId: number, providerId: number, customerName: string, oldRating: number, newRating: number) {
        try {
            const provider = await this.prisma.provider.findUnique({
                where: { id: providerId },
                select: { name: true, fcm: true }
     ...
     */
    async notifyNewService(serviceId: number, serviceName: string, providerName: string, category: string) {
        try {
            const notificationData: NotificationData = {
     ...
     */
    async notifyServiceUpdate(serviceId: number, serviceName: string, providerName: string, changes: string[]) {
        try {
            const notificationData: NotificationData = {
     ...
     */
    async notifyNewOffer(offerId: number, offerTitle: string, providerName: string, discount: string, serviceName: string) {
        try {
            const notificationData: NotificationData = {
     ...
     */
    async notifyOfferExpired(offerId: number, offerTitle: string, providerName: string) {
        try {
            const notificationData: NotificationData = {
                title: '⏰ Offer Expired',
     ...
     */
    async notifySystemMaintenance(message: string, scheduledTime?: string, duration?: string) {
        try {
            const notificationData: NotificationData = {
     ...
     */
    async sendGeneralAnnouncement(title: string, message: string, targetAudience: 'users' | 'providers' | 'all' = 'all') {
        try {
            const notificationData: NotificationData = {
                title,
                body: message,
