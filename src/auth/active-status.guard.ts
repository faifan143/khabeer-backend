    ...
    try {
      ...
      if (user.role === 'PROVIDER') {
        ...
        if (!providerRecord.isVerified) {
          throw new ForbiddenException(
            'Your provider account is not verified. Please wait for admin verification.',
          );
        }

        if (!providerRecord.isActive) {
          throw new ForbiddenException(
            'Your provider account is inactive. Please activate your account to access services.',
          );
        }
      }
